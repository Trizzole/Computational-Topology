...
inner_hole_pts <- tibble::tibble(x = c(2, 2, 7), y = c(2, 4, 4))
...
-   As $r$ increases, more simplices are included, until the Alpha complex becomes the full Delaunay triangulation.
:::
...
# --- Helper Function: Calculate Circumradius ---
circumradius <- function(pts) {
  ...
  if (is.nan(area) || abs(area) < 1e-9) return(Inf)
  return((a * b * c) / (4 * area))
}

# --- 1. Point and Parameter Setup ---
# A corrected, cohesive point set to robustly show all differences.
points_df <- tibble::tribble(
  ~x,     ~y,    ~name,
  # For VR > Cech (a very wide triangle)
   0.0,    0.0,    "A",
   1.95,   0.0,    "B",
   0.975,  0.2,    "C",
  # For Cech > Alpha (a Delaunay violation, positioned nearby)
   2.0,    2.9,    "D",
   1.22,   1.55,   "E",
   2.78,   1.55,   "F",
   2.0,    2.0,    "G"
...
delaunay_info <- deldir::deldir(points_df$x, points_df$y, suppressMsge = TRUE)
...
vr_tris <- all_tris |>
  dplyr::filter(
    dist_mat[cbind(p1, p2)] <= 2 * r &
    dist_mat[cbind(p1, p3)] <= 2 * r &
    dist_mat[cbind(p2, p3)] <= 2 * r
  )
...
create_complex_plot <- function(title, edge_df, tri_df) {
  edge_data <- edge_df |>
    tidyr::pivot_longer(cols = c(p1, p2), values_to = "id") |>
    dplyr::left_join(points_df, by = "id") |>
    dplyr::mutate(edge_id = rep(1:nrow(edge_df), each = 2))

  tri_data <- tri_df |>
    dplyr::select(p1, p2, p3) |>
    tidyr::pivot_longer(cols = c(p1, p2, p3), values_to = "id") |>
    dplyr::left_join(points_df, by = "id") |>
    dplyr::mutate(tri_id = rep(1:nrow(tri_df), each = 3))

  ggplot2::ggplot(points_df, ggplot2::aes(x = x, y = y)) +
    ggforce::geom_circle(ggplot2::aes(x0 = x, y0 = y, r = r), color = "gray80", linetype = "dashed") +
    ggplot2::geom_polygon(data = tri_data, ggplot2::aes(group = tri_id), fill = "skyblue", alpha = 0.6) +
    ggplot2::geom_line(data = edge_data, ggplot2::aes(group = edge_id), color = "black", linewidth = 1) +
    ggplot2::geom_point(size = 3.5, color = "#e41a1c") +
    ggplot2::geom_text(ggplot2::aes(label = name), nudge_y = 0.25, size = 3.5, fontface = "bold") +
    ggplot2::coord_fixed(xlim = c(-0.5, 3.5), ylim = c(-0.5, 3.5)) +
    ggplot2::theme_void() +
    ggplot2::theme(
        legend.position = "none",
...
::: {.notes}
Here's a breakdown of how these plots illustrate the key differences between the Alpha, Čech, and Vietoris-Rips complexes. The fundamental relationship is that for a given point set and radius $r$, the complexes are nested within each other: $\text{Alpha} \subseteq \text{Čech} \subseteq \text{Vietoris-Rips}$.

**1. Vietoris-Rips vs. Čech Complex**

* **The Rule:** A triangle (2-simplex) exists in the Vietoris-Rips (VR) complex if all three of its edges have a length less than or equal to $2r$. The Čech complex has a stricter requirement: a triangle exists only if the balls of radius $r$ centered at each vertex have a common intersection. This is equivalent to the triangle's circumradius being less than or equal to $r$.
* **The Example:** Look at triangle `ABC`. It is intentionally constructed to be wide and flat.
    * In the **Vietoris-Rips plot**, `ABC` is filled in because all its pairwise distances are less than $2r$.
    * In the **Čech plot**, `ABC` is *not* filled in. This is because its circumradius is much larger than $r$, so the three balls centered at A, B, and C do not have a common intersection. This single triangle demonstrates that the Čech complex can be a strict subset of the VR complex.

**2. Čech vs. Alpha Complex**

* **The Rule:** The Alpha complex is a subcomplex of the Delaunay triangulation. For a triangle to be included, it must first meet the Čech condition (circumradius $\le r$), *and* its circumcircle must be "empty," meaning it contains no other points from the set.
* **The Example:** Now, look at triangle `DEF` and the nearby point `G`.
    * In the **Čech plot**, triangle `DEF` is filled in. It was designed so its circumradius is less than or equal to $r$.
    * In the **Alpha plot**, `DEF` is *not* filled in. This is because point `G` was deliberately placed inside the circumcircle of `DEF`. This violates the "empty circumcircle" rule of the Delaunay triangulation. Therefore, even though it satisfies the Čech condition, it is excluded from the Alpha complex.
